...
(()=>{
    ...
    try{const data=await api('/projects/'+encodeURIComponent(selected)+'/native-threads/'+agent+'/'+encodeURIComponent(thread.id));
      ...
      const attach=button('선택',async()=>{if(!ack.checked)return;attach.disabled=true;
        try{if($('project').value!==selected||agent!==selectedImportClient)throw Error('프로젝트가 변경되었습니다. 다시 열어 주세요.');
          ...
          if($('project').value!==selected)throw Error('세션은 원래 프로젝트에 연결됐습니다. 해당 프로젝트에서 채팅을 선택하세요.');window.chatState.seed(selected,linked.task,{client:linked.client,cwd:'.',mode:'read-only',model:null,paths:[],fresh:false,action:'run'});importDialog.close();await switchChat(linked.task);actionHint();notice('대화를 연결했습니다. 다음 메시지부터 선택한 에이전트와 이어서 작업합니다.');
  ...
  const usageHeading=el('div',undefined,'usage-heading');usageHeading.append(el('h2','지금까지 사용한 토큰'));usage.append(usageHeading);
  ...
  usage.append(el('p','현재 프로젝트에서 기록한 사용량입니다. 계정의 남은 사용량이나 대화에 남은 공간과는 다릅니다.','hint'));
  ...
  usageDetails.append(el('p','에이전트마다 최근에 실행한 채팅 3개를 보여 줍니다. 완료된 채팅도 포함됩니다.','hint'));
  ...
  const accountDetail=button('모델과 사용 한도',async()=>accountDetails.showModal());accountDetail.title='모델과 사용 한도 보기';
  ...
  async function refreshEnvironment(){
    ...
    try{
      ...
      envStatus(failed?'error':'ready',failed?'일부 정보를 불러오지 못했습니다. 다시 시도해 주세요.':'확인 '+new Date().toLocaleTimeString());
    }catch(error){if($('project').value===projectName&&ticket===envTicket){for(const name of Object.keys(envStates))environmentState(name,'조회 실패','error');envStatus('error','정보를 불러오지 못했습니다. 다시 시도해 주세요.');}}finally{if(ticket===envTicket)envBusy=false;}
  }
  ...
  async function refreshAccount(){
    ...
    try{
      ...
      accountStatus(data.state==='error'?'error':'ready',data.state==='error'?'정보를 불러오지 못했습니다. 다시 시도할 수 있습니다.':'확인 '+new Date(data.checked_at).toLocaleTimeString());
      ...
      accountBody.append(el('p','목록 기본값은 실제 실행 모델을 뜻하지 않습니다. 실제 실행 모델과 대화 공간 사용량은 아직 확인할 수 없습니다.','hint'),el('p','계정 정보 출처: '+data.source+'. 마지막 확인: '+new Date(data.checked_at).toLocaleString()+'. 최대 60초 전 정보일 수 있습니다.','hint'));
    }catch(error){if(accountRows.textContent.includes('조회 중…'))accountRows.replaceChildren(accountPlaceholder('Codex','조회 실패'),accountPlaceholder('Claude','미수집'));accountStatus('error','정보를 불러오지 못했습니다. 다시 시도해 주세요.');}finally{statusBusy=false;}
  }
  ...
  const options=$('composer').querySelector('.composer-options');options.hidden=true;
  ...
  // Grow with the draft, including restored text and wrapping after width changes.
  const input=$('text');let resizeFrame=0,lastInputWidth=0;
  window.resizeComposer=()=>{
    cancelAnimationFrame(resizeFrame);
    resizeFrame=requestAnimationFrame(()=>{
      if(!input.getClientRects().length)return;
      const scroll=input.scrollTop,style=getComputedStyle(input);
  ...
  const inputSize=new ResizeObserver(entries=>{
    const width=entries[0].contentRect.width;
    if(width!==lastInputWidth){lastInputWidth=width;window.resizeComposer();}
  });
  inputSize.observe(input);window.addEventListener('resize',window.resizeComposer);window.resizeComposer();
  ...
  function renderNative(){
    ...
    if(!rows.length){native.textContent=filter?'아직 연결된 대화가 없습니다. Codex나 Claude가 첫 응답을 마치면 여기에 표시됩니다.':'채팅을 선택하면 연결된 에이전트 대화를 확인할 수 있습니다.';return;}
  ...
  async function refresh(){
    ...
    try{
      ...
      for(const run of next.running){const item=el('div');item.append(button((run.command.client||run.adapter)+' / '+run.project+' / '+(run.command.task||'미분류')+', '+(run.cancellable?'실행 중':'종료 확인 필요'),()=>openChat(run.project,run.command.task||''),'running-session'),el('p','대화 공간 사용량은 아직 확인할 수 없습니다.','hint'));list.append(item);}
    ...
    }catch(error){sessionStatus('error','정보를 불러오지 못했습니다. 다시 시도해 주세요.');}finally{inFlight=false;}
    try{await window.runNotifications.refresh();renderNotifications();}catch{alerts.title='알림을 불러오지 못했습니다. 다시 연결되면 확인합니다.';}
  }
  function renderNotifications(){
    const state=window.runNotifications.snapshot();alerts.textContent='알림 '+state.unread;alerts.title='최근 실행 종료 알림 100개';alertBox.replaceChildren();
