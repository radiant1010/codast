...
from app.core.rule_loader import COMMON_GUIDE_LABEL
...
def test_full_flow_and_context_selection(setup):
    ...
    assert [r['path'] for r in c.get('/api/projects/sample/rules').json()['rules']] == [COMMON_GUIDE_LABEL, 'RULES.md']
...
def test_rule_scope_reload_and_missing(setup):
    ...
    assert [r.path for r in agent.contexts[-1].rules] == [COMMON_GUIDE_LABEL,'RULES.md','source/RULES.md']
    (root/'RULES.md').unlink()
    assert [r['path'] for r in c.get('/api/projects/sample/rules').json()['rules']] == [COMMON_GUIDE_LABEL]


def test_common_guide_reaches_agent_and_is_fingerprinted(setup):
    import hashlib
    c, root, agent = setup
    original = (root/'RULES.md').read_text(encoding='utf-8')
    response = c.post('/api/projects/sample/commands',json={'text':'구조 검토'})
    assert response.status_code == 200
    guide = agent.contexts[-1].rules[0]
    assert guide.path == COMMON_GUIDE_LABEL
    assert '구조 변경은 보수적으로 처리한다' in guide.content
    assert '사용자 판단 필요' in guide.content
    run = c.get('/api/projects/sample/runs/'+response.json()['run_id']).json()
    assert run['metadata']['rules'][0] == {'path': COMMON_GUIDE_LABEL, 'sha256':hashlib.sha256(guide.content.encode('utf-8')).hexdigest()}
    assert (root/'RULES.md').read_text(encoding='utf-8') == original
